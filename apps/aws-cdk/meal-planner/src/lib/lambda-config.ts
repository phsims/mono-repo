...
export const getLambdaDefinitions = (appName: string, userPool?: UserPool): LambdaDefinition[] => {
    const environment = {
        DDB_TABLE: `${appName}`,
    };
...
export const getFunctionProps = (
    ...
    const functionProps: NodejsFunctionProps = {
        ...
        entry: path.join(__dirname, `../lambda-handlers/auth/${lambdaDefinition.name}.ts`),
