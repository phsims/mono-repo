...
// apps/aws-cdk/meal-planner/src/handlers/update-user.ts
...
var ddbWrite = (table, item) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      await ddbDocClient.send(new import_lib_dynamodb.PutCommand({ TableName: table, Item: item }));
      resolve();
    } catch (error) {
      reject(error);
    }
    ...
    try {
      ...
      if (validationResult.isValid) {
        ...
        if (getUserOutput.Items && getUserOutput.Items.length > 0) {
          const updatedUser = getUserOutput.Items[0];
          if (body.firstName)
          ...
          if (process.env.DDB_TABLE)
            await ddbWrite(process.env.DDB_TABLE, updatedUser);
          return resolve(
