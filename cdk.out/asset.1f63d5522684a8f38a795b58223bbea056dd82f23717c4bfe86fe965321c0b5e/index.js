...
// apps/aws-cdk/meal-planner/src/handlers/get-users.ts
...
var apiErrorResponse = (statusCode, error = "Something went wrong. Please contact administrator") => {
  return new Promise((resolve, reject) => {
    resolve({
      statusCode,
      headers: {
...
var getDDBDocClient = () => {
  return new Promise((resolve, reject) => {
    const ddbClient = new import_client_dynamodb.DynamoDBClient({ region: "ap-southeast-2" });
    ...
    try {
      ...
      if (validationResult.isValid) {
        ...
        const queryCommandInput = {
          ...
          ExclusiveStartKey: params.nextToken ? JSON.parse(
            Buffer.from(params.nextToken, "base64").toString("ascii")
          ) : void 0,
          ProjectionExpression: params.returnAttributes ? params.returnAttributes : void 0,
          Limit: params.limit ? params.limit : void 0,
        ...
        const queryCommandOutput = await ddbDocClient.send(
          new import_lib_dynamodb2.QueryCommand(queryCommandInput)
        );
        return resolve(
          await apiSuccessResponse({
            data: queryCommandOutput.Items ? import_lodash2.default.orderBy(queryCommandOutput.Items, "firstName", "asc") : [],
            nextToken: queryCommandOutput.LastEvaluatedKey ? Buffer.from(
        ...
        return resolve(
          await apiErrorResponse(400, validationResult.errors?.join(","))
        );
      }
    } catch (error) {
      logError(error);
